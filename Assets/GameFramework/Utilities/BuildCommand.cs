...
/// NB : QUESTO L HO SCARICATO DA INTERNET E HO COMMENTATO LA ROBA CHE AL MOMENTO NON SERVIVA, VA RIGUARDATO.
/// 
...
public class BuidCommand : MonoBehaviour
{
    ...
    private static string BuildPathRoot
    {
        get
        {
            string buildMainFolder = Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.Desktop), BuildFolder);
            string path = Path.Combine(buildMainFolder, ProductName);
            Debug.Log(path);
    ...
    static void DefaultBuild(BuildTarget buildTarget)
    {
        ...
        string path = Path.Combine(Path.Combine(BuildPathRoot, targetGroup.ToString()), ProductName + "_" + System.DateTime.Now.ToString("yyyy.MM.dd_HH") + "h" + System.DateTime.Now.ToString("mm") + "_" + buildTarget);
